...
Instagram DM自動化のオープンソースCRMツール。Cloudflare無料枠で動作します。
...
> Instagram DM 自動化を self-host したいチーム向けの OSS CRM。
> Cloudflare Workers / D1 / KV / R2 / Queues / Workflows を前提に構築しています。
...
セットアップCLI完了時に詳細な手順（URL・コマンド付き）が表示されます。概要：
...
- データ削除URL: `https://<your-worker>.workers.dev/api/data-deletion`
- 連絡先メールアドレス

### Step 10: アプリレビューへ移動

権限の用途説明テキスト（コピペ用）はCLI完了時に表示されます。
