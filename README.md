...
2. アプリ名に任意のアプリ名を入力して「次へ」
3. 左のフィルターで **「すべて」** を選び、**「Instagramでメッセージとコンテンツを管理」** のみにチェックを入れて「次へ」
4. 現時点ではビジネスポートフォリオをリンクしない
5. 公開の要件は「次へ」でスルー
6. 概要で「アプリを作成」

補足:
- ここで作成できない場合は、不要なアプリを削除する必要があります

### Step 2: ユースケースをカスタマイズ

ダッシュボードの概要 → 「Instagramでメッセージとコンテンツを管理」→ 「ユースケースをカスタマイズ」：
- **「Add all required permissions」** をクリック
- ボタンが **「Go to permissions and features」** に変わったら再度クリック
- アクセス許可と機能で `Business Asset User Profile Access` を追加
- `HUMAN_AGENT`（7日間の有人返信に必要）

### Step 3: テスターアカウントを追加

左メニュー「アプリの役割」→「役割」→ **「メンバーを追加」から Instagramテスターとして対象アカウントを追加**

### Step 4: Instagram側でテスター招待を承認

表示される
**「Instagramユーザーは、プロフィールのアプリとウェブサイトセクションから招待を管理できます。」**
のリンクからInstagram管理画面へ移動 → 「テスターへのご招待」タブ → 承認

補足:
- CLI に入力する `アプリID` と `App Secret` は `アプリの設定 → ベーシック` から取得します
...
### Step 5: アクセストークンを生成して登録

1. ダッシュボードの概要 → 「Instagramでメッセージとコンテンツを管理」→ 「ユースケースをカスタマイズ」
2. 「2. アクセストークンを生成する」を開く
3. テスターアカウントの「トークンを生成」をクリック
4. Instagram にログインして認証を許可
5. 「理解しました」にチェックを入れる
6. 表示されたアクセストークンと IG User ID をコピー
7. CLI に `Instagram User ID` と `アクセストークン` を入力
...
左メニュー「公開」→ 「プライバシーポリシーURLのアプリ設定に移動」→ アプリの設定 → ベーシック：
...
補足:
- その後、左メニューの「公開」を開き、右下の「公開」ボタンでアプリ全体を公開する
...
- 左メニュー「公開」を開き、右下の「公開」ボタンでアプリ全体を公開したこと
